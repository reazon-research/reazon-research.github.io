...
def _get_navigation_expand_image(soup: BeautifulSoup) -> Tag:
    retval = soup.new_tag("i", attrs={"class": "Tree__itemToggleIcon"})

    svg_element = soup.new_tag("svg")
    svg_use_element = soup.new_tag("use", href="#svg-arrow")
...
def get_navigation_tree(toctree_html: str) -> str:
    ...
    for element in soup.find_all("li", recursive=True):
        ...
        element["class"] = classes + ["Tree__item"]
        ...
        element["class"] += ["Tree__item--has-children"]
        ...
        label = soup.new_tag(
            "label", attrs={"for": checkbox_name, "class": "Tree__itemToggle"})
