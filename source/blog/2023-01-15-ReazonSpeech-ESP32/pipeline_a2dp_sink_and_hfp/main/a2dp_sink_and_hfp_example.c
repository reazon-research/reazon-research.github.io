...
static uint32_t bt_app_hf_client_outgoing_cb(uint8_t *p_buf, uint32_t sz)
{
    ...
    if (is_get_hfp) {
        ...
        reazon_speech_callback((uint8_t *)enc_buffer, out_len_bytes, 0);
    }
...
{
    reazon_speech_callback(buf, sz, 1);
...
void app_main(void)
{
    ...
    // H.Shirouzu
    wifi_start();
    xTaskCreate(&reazon_speech_maintask, "reazon_speech_maintask", 2048, NULL, 5, NULL);
